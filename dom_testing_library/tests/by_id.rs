...
pub fn by_id() {
    ...
    assert_eq!(
        document.get_by_id("div_3"),
        Err(Error::NotFound {
            ident: "div_3".to_string(),
            method: "by_id".to_string()
        })
    );
    assert_ne!(
        document.get_by_id("div_1").unwrap(),
        (document.get_by_id("div_2").unwrap())
    );
    assert_eq!(document.get_all_by_id_contains("div").len(), 2);
}
